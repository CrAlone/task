...
(function ($) {
	...
	function show(target) {
		...
		for (var i = 0; i < weeks.length; i++) {
			...
			for (var j = 0; j < week.length; j++) {
				...
				var d_day = day[2];
				//任务表单id
				for (var k = 0;k<allTask.length;k++){
					//取出一个任务判断是是否和日期相同
					var task = allTask[k];
					if (task.info_create_date == y_year+"-"+(m_month<10?("0"+m_month):m_month)+"-"+(d_day<10?("0"+d_day):d_day)){
						dayHtml +="<br'><div style='background: #ccc;height: 5%;width: 100%;text-align: center;font-size: 3px;'></div>";
						//找到一个count++
						taskCount++;
						//将要展示的信息找寻出来
						//获取id
						var info_id = task.info_id;
						//任务名称
						var info_name = task.info_name;
						//任务级别
						//判断任务级别
						var level_name = task.level.level_name;
						//任务创建时间
						var info_create_date = task.info_create_date;

						//待完成时间
						var	info_end_not_ate = task.info_end_not_ate;
						//派发任务的人
						var staff_name = task.staff.staff_name;
						//接受任务的人
						var tas_staff_name = task.taskStaff.staff_name;
						//任务状态
						var state_name = task.state.state_name;
						//任务介绍
						var info_desc = task.info_desc;
						//赋值给表单
						//任务创建
						$("#text1").val(info_create_date);
						// //待完成时间
						$("#text2").val(info_end_not_ate);
						// //派发任务的人
						$("#text3").val(staff_name);
						// //任务名称
						$("#text4").val(info_name);
						// //接受任务的人
						$("#text5").val(tas_staff_name);
						// //任务级别
						$("#text6").val(level_name);
						// //任务状态
						$("#text7").val(state_name);
						//任务说明
						$("#text8").val(info_desc);
						...
						if (level_name == "普通"){
							color = "#111";
						}else if (level_name == "重要"){
							color = "#0000ff";
						}else if (level_name == "紧急"){
							color = "#ff0000";
						}
						...
						// newA.setAttribute("style","'font-size: 3px;color:"+color+";float:left'")
						// newA.innerHTML = taskCount+":"+name;
						// box.prepend(newA);

						//
						dayHtml +="<br'><a id='mdetails' href='#' class='click' style='font-size: 3px;color:"+color+";float: left;text-decoration: none;' title='点击详细内容'><input hidden='true' value='"+info_id+"'>"+taskCount+"："+info_name+"</a>";
		...
		// }
				...
				day.dblclick(function (e) {
					clearTimeout(hideTimer);
					 var inf = $(this).data('info');
					$("#panel").dialog('open').dialog("setTitle","<span style='color:blue;font-family: 微软雅黑;'>新增任务</span>");
					$("#info_create_date").val(inf.sYear+"-"+(inf.sMonth<10?("0"+inf.sMonth):inf.sMonth)+"-"+(inf.sDay<10?("0"+inf.sDay):inf.sDay));
				});
	...
	$('head').append('<style>.fullcalendar-day {border-left: 1px dotted green;border-top: 1px dotted green;} '
		 ...
		 + '.fullcalendar-detail .lunarFestival{color: red;}.fullcalendar-detail .solarFestival{color: blue;} .calendar-selected {border: 1px solid red;};background-repeat: no-repeat;background-position: top right;}</style>');
